...
try {
    ...
    if ($PreDeployment -eq $true) {
        ...
        # DEBUG: Inspect template triggers and selection criteria
        Write-Host ("[DEBUG] updatedTriggersInTemplate count: {0}" -f ($updatedTriggersInTemplate | Measure-Object | Select-Object -ExpandProperty Count))
        $updatedTriggersInTemplate | ForEach-Object {
            $name = $_.name.Substring(37, $_.name.Length - 40)
            $desired = $_.properties.runtimeState
            $hasPipelines = ($_.properties.pipelines.Count -gt 0) -or ($_.properties.pipeline.pipelineReference -ne $null)
            $isRunning = $name -in $triggersRunning
            Write-Host ("[DEBUG] template trigger: name={0}, type={1}, desiredRuntimeState={2}, hasPipelines={3}, currentlyRunning={4}" -f $name, $_.Properties.type, $desired, $hasPipelines, $isRunning)
        }
        ...
        if ($triggersToStart.Count -gt 0) {
            ...
            Write-Host ("[DEBUG] triggersToStart: {0}" -f ($triggersToStart | ForEach-Object Name -join ', '))
        } else {
            Write-Host "[DEBUG] No triggers selected to start. Check desired runtimeState in template, pipeline references, and running state."
        }

        $triggersToStart | ForEach-Object {
            $tName = $_.Name
            $tType = $_.TriggerType
            Write-Host ("[DEBUG] Processing trigger: name={0}, type={1}" -f $tName, $tType)
            if ($tType -eq 'BlobEventsTrigger') {
                Write-Host "Subscribing $tName to events"
                $status = Add-AzDataFactoryV2TriggerSubscription -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name $tName
                Write-Host ("[DEBUG] Initial subscription status for {0}: {1}" -f $tName, $status.Status)
                while ($status.Status -ne 'Enabled') {
                    Start-Sleep -s 15
                    $status = Get-AzDataFactoryV2TriggerSubscriptionStatus -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name $tName
                    Write-Host ("[DEBUG] Waiting for subscription to enable for {0}. Current status: {1}" -f $tName, $status.Status)
                }
                Write-Host ("[DEBUG] Subscription Enabled for {0}" -f $tName)
            }
            Write-Host ("Starting trigger {0}" -f $tName)
            Write-Host ("[DEBUG] Start-AzDataFactoryV2Trigger -ResourceGroupName {0} -DataFactoryName {1} -Name {2} -Force" -f $ResourceGroupName, $DataFactoryName, $tName)
            Start-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name $tName -Force
            # Verify post-start state
            try {
                $post = Get-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name $tName
                Write-Host ("[DEBUG] Post-start RuntimeState for {0}: {1}" -f $tName, $post.RuntimeState)
            } catch {
                Write-Host ("##[warning] Unable to read post-start state for {0}. $_" -f $tName)
            }
        }
    }
} catch {
